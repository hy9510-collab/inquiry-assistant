...
// 키는 Vercel 환경변수 GEMINI_API_KEY 에서만 읽는다(브라우저에 노출 안 됨).
...
async function callGemini(parts) {
  ...
  if (!key) throw new GeminiError("NO_KEY: 서버에 AI 키(GEMINI_API_KEY)가 설정되지 않았습니다.");
...
// 텍스트 원문 → 정리
export async function organizeText(text, sourceName = "", wantFormat = "", note = "") {
  const hint = sourceName ? `\n\n[자료 출처] ${sourceName}` : "";
  const ask = note ? `\n\n[추가 요청사항] ${note}` : "";
  const prompt = ORGANIZE_PROMPT + hint + ask + "\n\n[자료 원문]\n" + String(text).slice(0, 120000);
  const doc = await callGemini([{ text: prompt }]);
...
export async function organizeYoutube(url, wantFormat = "", note = "") {
  ...
  let doc;
  try {
    doc = await callGemini([{ text: prompt }, { fileData: { fileUri: clean } }]);
  } catch (e) {
...
export async function organizeFile(base64, mimeType, sourceName = "", wantFormat = "", note = "") {
  const ask = note ? `\n[추가 요청사항] ${note}` : "";
  const prompt = ORGANIZE_PROMPT + `\n\n[자료 출처] ${sourceName}${ask}\n\n첨부된 자료의 내용을 위 스키마로 정리하십시오.`;
  const doc = await callGemini([{ text: prompt }, { inline_data: { mime_type: mimeType, data: base64 } }]);
  return normalizeDoc(doc, sourceName, wantFormat);
}
